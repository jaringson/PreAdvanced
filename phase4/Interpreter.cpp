...
{
	Rule rulecopy = rule;
	for (int i = 1; i < rulecopy.body.size(); i++)
	{
		ruleSelection(rulecopy.body[i], i);
	...
	for (int i = 2; i < rulecopy.body.size(); i++)
	{
		
		Predicate r2 = rulecopy.body[i];
		pos0 = position(r, r2);
		//pos1 = position(r2, r);
	...
	Predicate result_pred = rulecopy.body[1];
	for (int i = 2; i < rulecopy.body.size(); i++)
	{
		if (i != 2)
			res1 = result;
	...
	vector<int> pos = position(rulecopy.body[0], r);
	...
	for (set<Tuple>::iterator it = result.set_relation.begin(); it != result.set_relation.end(); ++it)
	{
		...
		if (one.size() == greatest)
		{
			...
			//cout << "Here 14  " << endl;
			data.database[rulecopy.body[0].name].set_relation.insert(t);
		}
...
void Interpreter::ruleSelection(Predicate& pred, int& num)
{
	...
	id.clear();
	data.database[to_string(num)] = relation;
	//cout << "       " << data.database.size() << " " << data.database[to_string(i)].set_relation.size() << endl;
...
{
	int count = 0;
	if(queries[0].name == "permutations" || queries[0].name == "slides" )
	{ 
		count = -1;
	}
